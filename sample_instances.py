...
context_length_left = 255
context_length_right = 254
...
with open("samples.csv", "w") as outfile:
	outfile.write("index,filename,index_in_file,kr_first,kr_second,kr_third,context_left,context_right\n")
	
	index = 0
	
	for filename, text in tqdm.tqdm(rsts):
		yi_indices = [i for i, c in enumerate(text) if c == "一"]
		
		kr_number = filename.split("/")[-1].split("_")[0]
		
		for yi_index in yi_indices:
			
			context_left = text[max(0, yi_index - context_length_left) : yi_index]
			context_right = text[yi_index + 1 : yi_index + 1 + context_length_right]
			
			assert(len(context_left) <= context_length_left)
			assert(len(context_right) <= context_length_right)
			
			outfile.write(",".join((str(index), filename, str(yi_index), 
				kr_metadata[kr_number[:3]], kr_metadata[kr_number[:4]],
				kr_metadata[kr_number], context_left, context_right)) + "\n")
			index += 1
